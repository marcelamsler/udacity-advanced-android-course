...
public class Utility {
    ...
    public static String getArtUrlForWeatherCondition(Context context, int weatherId) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        String artPackUrl = prefs.getString(context.getString(R.string.pref_art_pack_key),
                context.getString(R.string.pref_art_pack_sunshine));

        // Based on weather code data found at:
        // http://bugs.openweathermap.org/projects/api/wiki/Weather_Condition_Codes
        if (weatherId >= 200 && weatherId <= 232) {
            return String.format(artPackUrl, "storm");
        } else if (weatherId >= 300 && weatherId <= 321) {
            return String.format(artPackUrl, "light_rain");
        } else if (weatherId >= 500 && weatherId <= 504) {
            return String.format(artPackUrl, "rain");
        } else if (weatherId == 511) {
            return String.format(artPackUrl, "snow");
        } else if (weatherId >= 520 && weatherId <= 531) {
            return String.format(artPackUrl, "rain");
        } else if (weatherId >= 600 && weatherId <= 622) {
            return String.format(artPackUrl, "snow");
        } else if (weatherId >= 701 && weatherId <= 761) {
            return String.format(artPackUrl, "fog");
        } else if (weatherId == 761 || weatherId == 781) {
            return String.format(artPackUrl, "storm");
        } else if (weatherId == 800) {
            return String.format(artPackUrl, "clear");
        } else if (weatherId == 801) {
            return String.format(artPackUrl, "light_clouds");
        } else if (weatherId >= 802 && weatherId <= 804) {
            return String.format(artPackUrl, "clouds");
        }
        return null;
    }
